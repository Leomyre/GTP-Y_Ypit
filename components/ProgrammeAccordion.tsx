...
export function ProgrammeAccordion({ programme, isResponsable = false, voyageId }: Props) {
            ...
            {programme.map((jour) => (
                <div key={jour.id} className="border rounded-lg overflow-hidden shadow-sm bg-gray-50 dark:bg-gray-800 transition-all">
                    ...
                    >
                        ...
                        {openJour === jour.id ? <ChevronUp className="w-5 h-5" /> : <ChevronDown className="w-5 h-5" />}
                    </button>

                    {openJour === jour.id && (
                        <div className="p-4 pt-0 text-sm space-y-2 text-muted-foreground">
                            {jour.activite && <p><strong>Activité :</strong> {jour.activite}</p>}
                            {jour.lieu && <p><strong>Lieu :</strong> {jour.lieu}</p>}
                            {jour.repas_inclus && <p><strong>Repas :</strong> {jour.repas_inclus}</p>}
                            ...
                            {isResponsable && (
                                <div className="pt-3">
                                    <Button
                                        ...
                                        size="sm"
                                        onClick={() =>
                                            window.location.href = `/responsable/tour/programme/${jour.id}/modifier?voyageId=${voyageId}`
                                        }
                                    >
                                        Modifier ce jour
                                    </Button>
